...
from api.core.enums import ReportingPeriod
...
def filter_by_query_param(queryset, query, search_fields, lookup_field=None):
    ...
    if query:
        query_by = {
            "{0}{1}__{2}".format(f"{lookup_field}__"if lookup_field else "", field, "icontains"): \
                query for field in search_fields
        }
        query_objects = Q()
        for q in query_by:
